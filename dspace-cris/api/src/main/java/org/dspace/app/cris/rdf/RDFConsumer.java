...
import org.dspace.app.cris.rdf.RDFConsumer;
...
public class RDFConsumer implements Consumer
{
    ...
    public void consumeACO(Context ctx, Event event) throws SQLException
    { //Consumes ACrisObject (possible?)
    	//not tested
        if (event.getSubjectType() != CrisConstants.RP_TYPE_ID) //extend for other types
        {
            log.error("Called on an unexpected Event with subject type " 
                    + event.getSubjectTypeAsString() + " and event type " 
                    + event.getEventTypeAsString() + ", ignoring.");
                ...
                || event.getEventType() == Event.CREATE)
        {
            ...
            //handle not working here ? -> applicationService with crisid possible?
            DSOIdentifier id = new DSOIdentifier(dso, ctx);
