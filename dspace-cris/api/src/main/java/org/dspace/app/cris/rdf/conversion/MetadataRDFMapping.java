...
import org.dspace.app.cris.rdf.conversion.MetadataMappingException;
...
public class MetadataRDFMapping {
    ...
    protected void compileResult(Model m, Resource result, 
            String dsoIRI, String name, Map<String,String> value, String lang) throws MetadataMappingException
    {
    ...
    protected Resource parseSubject(Model m, Resource subject, String dsoIRI, 
            String name, Map<String,String> value)
    {
    ...
    protected RDFNode parseObject(Model m, RDFNode objectNode, String dsoIRI,
            String name, Map<String,String> value, String lang)
    {
    ...
    protected String parseResourceGenerator(Resource resourceGenerator, 
            Map<String,String> value, String dsoIRI)
    {
    ...
    protected Literal parseLiteralGenerator(Model m, Resource literalGenerator, 
            Map<String,String> value, String lang)
    {
        if (literalGenerator.isURIResource() 
                && literalGenerator.equals(DMRM.DSpaceValue))
        {
            return m.createLiteral(value.get("DSpaceValue"));
        }
    ...
    protected String parseValueProcessor(Resource valueProcessor, Map<String,String> value)
    {
        ...
        if (modifierNode != null)
        {
            ...
            try
            {
                ...
                String modifiedValue = pattern.matcher(value.get("DSpaceValue")).replaceAll(replacement);

                log.debug("Found matcher '" + matcher + "'.\n"
                    + "Found replacement '" + replacement + "'.\n"
                    + "modified '" + value + "' => '" + modifiedValue + "'.");

                value.put("DSpaceValue", modifiedValue);
            }
            catch (PatternSyntaxException ex)
            {
        ...
        String result = pattern.replace("$DSpaceValue", value.get("DSpaceValue"));
        if (result.contains("$DSpaceAuthority"))
        {
        	if (!value.containsKey("DSpaceAuthority") || value.get("DSpaceAuthority") == null)
        	{
        		return null;
        	}
        	result = result.replace("$DSpaceAuthority", value.get("DSpaceAuthority"));
        }
