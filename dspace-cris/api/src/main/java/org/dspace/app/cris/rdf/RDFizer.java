...
import org.dspace.app.cris.rdf.ItemNotArchivedException;
import org.dspace.app.cris.rdf.ItemNotDiscoverableException;
import org.dspace.app.cris.rdf.ItemWithdrawnException;
import org.dspace.app.cris.rdf.RDFConfiguration;
import org.dspace.app.cris.rdf.RDFMissingIdentifierException;
...
//import org.dspace.app.cris.integration.CrisService;
...
public class RDFizer {
            ...
            throws SQLException
    {
        if (dso.getType() != Constants.SITE
                ...
                && dso.getType() != CrisConstants.RP_TYPE_ID 
                && dso.getType() != CrisConstants.PROJECT_TYPE_ID
                && dso.getType() != CrisConstants.OU_TYPE_ID
                && !(dso.getType() > CrisConstants.CRIS_DYNAMIC_TYPE_ID_START))
        {
        ...
        Callback callback = new Callback() {
                    ...
                    throws SQLException
            {
                ...
                log.debug("Trying to delete dspaceobject " + identifier + " (Callback)");
...
//Callback seems not to work for ACrisObject (Delete from URI). Deleting from jspui worked?
        if ( dso.getType() == CrisConstants.RP_TYPE_ID 
                || dso.getType() == CrisConstants.PROJECT_TYPE_ID
                || dso.getType() == CrisConstants.OU_TYPE_ID
                || (dso.getType() >= CrisConstants.CRIS_DYNAMIC_TYPE_ID_START)) {
        	String identifier = RDFUtil.generateIdentifier(context, dso);
            ...
            throws SQLException
    {
        ...
        Callback callback = new Callback() {
                    ...
                    throws SQLException
            {
                ...
                try
                {
                	log.debug("inside Callback: " + dso.getName());
                    if (dryrun)
                    {
                    ...
                    if (!(dso instanceof Item) || !(dso instanceof ACrisObject)) throw new IllegalStateException(ex.getMessage(), ex);
        ...
        //Callback seems not working for Converting of CrisObjects -> don't know why. Seems to work in 5.10
        //FIXME: running procedure inside the Callback in a own loop
        if(dso.getType() == CrisConstants.RP_TYPE_ID 
        || dso.getType() == CrisConstants.PROJECT_TYPE_ID
        || dso.getType() == CrisConstants.OU_TYPE_ID
        || (dso.getType() >= CrisConstants.CRIS_DYNAMIC_TYPE_ID_START)){
             ...
             try
             {
             	log.debug("inside CRIS-Callback: " + dso.getName());
                 if (dryrun)
                 {
                     converted = RDFUtil.convert(context, dso);
                 } else {
                 ...
                 if (!(dso instanceof ACrisObject) || !(dso instanceof DSpaceObject)) throw new IllegalStateException(ex.getMessage(), ex);
                 report("Skipping conversion of Object " + dso.getID() 
                         + " (handle " + dso.getHandle() + "): Object is not "
                         + "archived.");
                 return;
             } catch (ItemWithdrawnException ex) {
                 if (!(dso instanceof ACrisObject) || !(dso instanceof DSpaceObject)) throw new IllegalStateException(ex.getMessage(), ex);
                 report("Skipping conversion of Object " + dso.getID() 
                         + " (handle " + dso.getHandle() + "): Object is "
                         + "withdrawn.");
                 return;
             } catch (ItemNotDiscoverableException ex) {
                 if (!(dso instanceof ACrisObject) || !(dso instanceof DSpaceObject)) throw new IllegalStateException(ex.getMessage(), ex);
...
//        }
        ...
        if (line.hasOption("delete"))
        {
            ...
            for (String identifier : identifiers)
            {
                ...
                if (dso == null) {
                	...
                	log.debug("No identifier " + handle + ".");
                }
                ...
                try
                {
                    ...
                    log.debug("Deleting Object " + dso.getID() + " | " + dso.getHandle());
                }
    ...
    { //resolveCrisID instead of handle for CRIS-Elements
        ...
        if (dso.getType() != Constants.SITE
                ...
                && dso.getType() != CrisConstants.PROJECT_TYPE_ID
                && dso.getType() != CrisConstants.OU_TYPE_ID
                && !(dso.getType() > CrisConstants.CRIS_DYNAMIC_TYPE_ID_START))//allow all dynamic objects
        		//&& !CrisConstants.getEntityTypeText(dso.getType()).equalsIgnoreCase("CRISevent")) //dynamic object check CRIS+shortname
        {
