...
public class RDFUtil {
            ...
            AuthorizeException, IllegalArgumentException
    {
        ...
        {
        	log.debug(dso.getTypeText() + " Type is not supported in RDfConfiguration");
            return null;
        }
     ...
     * {@link org.dspace.app.cris.rdf.storage.RDFStorage RDFStorage}. 
     * Please note that dspace-rdf doesn't support Bundles or Bitstreams as 
     * independent entity. You can convert DSpaceObjects of type SITE,
     * COMMUNITY, COLLECTION or ITEM.
     * @param context Consider that the converted data will be stored in a
            ...
            ItemNotDiscoverableException, AuthorizeException
    {
        ...
        //commented out for CRIS- troubles occured with to authorize the Action
        ...
        //CRIS-Objects: cannot be configured (e.g.Withdrawn), but method is available
        if(dso instanceof ACrisObject) {
            ACrisObject aco = (ACrisObject) dso;
            //if (!aco.isArchived()) throw new ItemNotArchivedException(); //isArchived = isDiscoverable for ACrisObject
            if (!aco.isDiscoverable()) throw new ItemNotDiscoverableException();
            if (aco.isWithdrawn()) throw new ItemWithdrawnException();
